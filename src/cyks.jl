...
function CykModel(ctb::ChineseTreeBank)
    labelid = Td{String, Int}()
    falr = Td{Tuple{Int, Int, Int}, Tf}()
    fass = Td{Tuple{Int, Int}, Tf}()
    function visitor(cur::ChineseTree)::Nothing
        nchd = length(cur.adj)
...
function cyk(poswords::Vector{Tuple{String, String}}, model::CykModel)
    ...
    function dfs(s, o, ilabel)::ChineseTree
        cur = ChineseTree(model.idlabel[ilabel], ChineseTree[])
        nxt = nx[s, o][ilabel]; nnxt = length(nxt)
        if nnxt == 1
            push!(cur.adj, ChineseTree(nxt[1], ChineseTree[]))
        elseif nnxt == 2
