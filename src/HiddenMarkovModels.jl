...
function chmm()
    poswords = joinpath(dirname(pathof(ChineseTokenizers)), "..", "data", "posword")
    userdict = joinpath(dirname(pathof(ChineseTokenizers)), "..", "data", "userdict")
    return HiddenMarkovModel{Float32, Int32}(; poswords = poswords, userdict = userdict, model = nothing)
end
